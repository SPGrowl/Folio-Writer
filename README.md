...
基于 [ChatGPT-Web](https://github.com/Chanzhaoyu/chatgpt-web) 二次开发的轻量 AI 对话应用。保留多会话、本地持久化、Markdown 渲染、模型切换等核心能力，并对**数据模型**、**流式协议**与**组件架构**做了重构，更贴近 OpenAI / DeepSeek 等官方 API 与主流产品交互。

> 一期目标：跑通「多轮对话 + SSE 流式 + 思考内容展示 + 本地会话管理」主路径。

## 特性

- **Session / Turn 会话模型**：`sessionStore` 统一管理会话与轮次，替代原版 `history + chat` 双数组
- **标准 SSE 流式对话**：前端 `fetch + ReadableStream` 解析 SSE，后端 OpenAI Node SDK 透传分片
- **思考过程展示**：流式写入并折叠展示 `reasoning_content`
- **Bubble 对话气泡**：用户消息编辑重发、助手回复复制/重试、悬停操作栏
- **Home 起始页**：独立欢迎页与输入区，创建会话后自动发起首轮流式请求
- **本地持久化**：会话数据保存在浏览器本地存储
